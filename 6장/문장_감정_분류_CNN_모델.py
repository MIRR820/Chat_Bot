...
features = data['Q'].tolist()           # features 리스트에 질문 목록들을 추가
labels = data['label'].tolist()         # labels 리스트에 감정을 저장
...
test_size = int(len(padded_seqs)*0.1)
                                            # 해당 배치를 매개 변수 번 불러온다.
train_ds = ds.take(train_size).batch(20)    # batch는 1배치 당 20개의 데이터 크기
val_ds = ds.skip(train_size).take(val_size).batch(20)
test_ds = ds.skip(train_size + val_size).take(test_size).batch(20)

# 하이퍼파라미터 설정
dropout_prob = 0.5                  # 50% 드랍아웃 발생  #  드랍아웃 연결된 신경을 버리는 행위 과대적합을 방지
